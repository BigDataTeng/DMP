...
object QuipmentCt {
  def main(args: Array[String]): Unit = {
    ...
    // 指标统计
    //    sQLContext.sql("select devicetype from log").show()
    val res = sQLContext.sql("select " +
      "case when devicetype=1 then '手机' when devicetype=2 then'平板' else '其他' end as devicetype," +
    ...
    res.write.mode(SaveMode.Append).
      jdbc(load.getString("jdbc.url"),"QuipmentCt",prop)
